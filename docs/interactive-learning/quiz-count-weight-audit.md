...
| Day02 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | Prompt 合约、RAG 判断、检索评估、grounding、拒答策略都值得单独考。 | 暂不改；建议后续补 1 题“引用校验/拒答边界”。 |
| Day03 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | Agent、Workflow、state、tool permission、人类确认/恢复是五个关键能力点。 | 暂不改；建议后续补 1 题“checkpoint/resume 诊断”。 |
| Day04 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | 产品闭环应覆盖 UX、模型网关、streaming、trace/eval、部署/成本。 | 暂不改；建议后续补 1 题“部署/回滚或成本门禁”。 |
...
| Day09 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | 生产 RAG 密度高，建议把 rerank/eval 或漂移监控拆出一题。 | 暂不改；建议后续补 1 题。 |
| Day10 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | Eval、rubric、反馈、红队、发布门禁都应被测。 | 暂不改；建议后续补 1 题“发布阈值/门禁”。 |
| Day11 | 4 题，无显式权重 | 4 | 30/25/20/25 | 当前四题覆盖上下文、拆解、prompt 资产、验证闭环；适合保留。 | 暂不改；只需研发支持 4 题权重。 |
| Day12 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | Capstone 内容密度高，建议增加 eval/证据或 fallback 题。 | 暂不改；建议后续补 1 题。 |
| Day13 | 4 题，无显式权重 | 4 | 25/25/25/25 | 当前四题覆盖设计数据、多域检索、设计系统、UI 验收；适合保留。 | 暂不改；只需研发支持 4 题权重。 |
| Day14 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | 数据治理应覆盖契约、新鲜度、删除传播、引用、权限过滤。 | 暂不改；建议后续补 1 题“权限过滤位置”。 |
| Day15 | 4 题，无显式权重 | 5 | 25/20/20/20/15 | 安全章节应覆盖不可信输入、schema、策略、审批、审计/红队。 | 暂不改；建议后续补 1 题“审批/红队回归”。 |
| Day16 | 4 个开放问题，无答案/权重 | 5 | 25/20/20/20/15 | 多模态应覆盖异步队列、证据锚点、降级、成本/延迟、隐私/输入契约。 | 建议内容后续结构升级；本轮只审计。 |
| Day17 | 4 个开放问题，无答案/权重 | 5 | 25/20/20/20/15 | 成本容量应覆盖缓存、路由、重试预算、p95、预算/SLO 降级。 | 建议内容后续结构升级；本轮只审计。 |
| Day18 | 4 个开放问题，无答案/权重 | 5 | 25/20/20/20/15 | SRE 应覆盖 SLO、trace、熔断、回滚、runbook/复盘。 | 建议内容后续结构升级；本轮只审计。 |
| Day19 | 4 个开放问题，无答案/权重 | 5 | 25/20/20/20/15 | 发布治理应覆盖 trace 版本、资产版本化、门禁、灰度阻断、owner。 | 建议内容后续结构升级；本轮只审计。 |
| Day20 | 4 个开放问题，无答案/权重 | 5 | 25/20/20/20/15 | 产品飞轮应覆盖能力证据、北极星、护栏、实验停止条件、90 天路线图。 | 建议内容后续结构升级；本轮只审计。 |
...
- Day01 已经是 5 题。
- Day02、Day03、Day04、Day09、Day10、Day12、Day14、Day15、Day16、Day17、Day18、Day19、Day20。
...
- Day02-Day15 typed lesson 均已有可交互选择题、答案和解释；本轮补齐显式权重，使每章合计 100。
- Day16-Day20 typed lesson 已有完整选择题、答案和解释；本轮补齐显式权重，使每章合计 100。
